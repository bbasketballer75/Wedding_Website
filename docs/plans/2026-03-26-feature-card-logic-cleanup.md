...
```tsx
{
  guestHighlights.length > 0 && <section className='px-4 pb-16'>...</section>
}
```
...
```tsx
<div className='mb-5'>
  <h2 className='font-display text-3xl text-charcoal-900 sm:text-4xl'>{selectedCollection}</h2>
</div>
```
...
try {
  ...
  if (!rpcError && rpcData) {
    ...
    const { data, error } = await supabase
      .from('guestbook_messages')
      .insert([
        { name, email, content: normalizedContent, type: 'text', media_url: null, reactions: {} },
      ])
      .select()
...
const filteredMessages = messages.filter(message =>
  filter === 'all' ? true : message.type === filter
)
...
<div className='relative'>
  ...
  <p className='mt-5 max-w-2xl text-base text-white/55 sm:text-lg'>
    Phone photos, shaky dance-floor videos, ceremony candids, quiet table moments: the whole archive
    gets better when your side of the day is part of it too.
  </p>
...
>
  <div className='flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between'>
    <div className='max-w-xl'>
      <p className='text-[10px] uppercase tracking-[0.3em] text-gold-400'>Pass the site along</p>
