...
const seoExpectations = [
  ...
  { route: '/film', title: 'Wedding Film', canonicalSuffix: '/film', robots: 'index, follow' },
  {
    route: '/gallery',
    title: 'Photo Gallery',
    canonicalSuffix: '/gallery',
    robots: 'index, follow',
  },
  {
    route: '/guestbook',
    title: 'Guestbook',
    canonicalSuffix: '/guestbook',
    robots: 'index, follow',
  },
  {
    route: '/upload',
    title: 'Share Memories',
    canonicalSuffix: '/upload',
    robots: 'index, follow',
  },
  ...
  for (const expectation of seoExpectations) {
    test(`applies metadata for ${expectation.route}`, async ({ page }) => {
      ...
      await expect(page).toHaveTitle(
        new RegExp(`^${expectation.title} \\| Austin & Jordyn's Wedding$`)
      )
      ...
      await expect(canonical).toHaveAttribute(
        'href',
        new RegExp(`${expectation.canonicalSuffix.replace('/', '\\/')}$`)
      )
