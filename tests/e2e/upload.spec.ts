...
test.describe('Upload Page', () => {
  test('accepts fixture media and shows stable success and error queue states', async ({
    page,
  }) => {
  ...
  test('submits a successful upload and renders the completion panel', async ({ page }) => {
    ...
    const fileInput = page.locator('input[type="file"]')
    await fileInput.setInputFiles([
      { name: 'table-candid.png', mimeType: 'image/png', buffer: tinyPng },
    ])
...
for (const viewport of Object.keys(viewports) as Array<keyof typeof viewports>) {
  test(`upload visual baselines (${viewport})`, async ({ page }) => {
    ...
    const successPanel = page.getByTestId('upload-success-panel')
    await successPanel.waitFor({ state: 'visible', timeout: 10000 })
    await page.waitForTimeout(300) // let animations settle
    await expectSectionScreenshot(successPanel, `upload-success-${viewport}.png`)
  })
}
