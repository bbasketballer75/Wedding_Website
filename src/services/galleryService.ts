...
class GalleryService {
  ...
  async getImages(options: GetImagesOptions = {}): Promise<GalleryImage[]> {
    const {
      category = null,
      tags = [],
      featured = null,
  ...
  // Get featured images
  async getFeaturedImages(limit: number = 10): Promise<GalleryImage[]> {
  ...
  // Search images
  async searchImages(query: string, options: { limit?: number } = {}): Promise<GalleryImage[]> {
    try {
      const { data, error } = await supabase
        .from('wedding_gallery')
        .select(
