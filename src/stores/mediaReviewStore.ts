...
function isGuestReviewBatch(batch: MediaReviewBatch) {
  return (
    batch.batch_key.startsWith('guest-review-batch-') ||
    batch.notes === 'Guest upload face review batch'
  )
}
...
function getFaceLabel(face: MediaReviewFace) {
  return (
    face.confirmed_name ||
    getMetadataValue<string>(face, 'suggestedLabel') ||
    face.cluster_id ||
    face.face_id
  )
}
...
function buildPhotoRecords(faces: MediaReviewFace[], importRows: ReviewImportManifestRow[]) {
  ...
  for (const face of faces) {
    ...
    const record: ReviewPhotoRecord = existing || {
      ...
      sourceRelativePath:
        face.source_relative_path || importRow?.sourceRelativePath || 'Unknown source',
      photoUrl: face.photo_url || (importRow ? toSiteMediaPath(importRow.photoRowDraft.url) : ''),
      thumbnailUrl:
        face.thumbnail_url ||
        (importRow ? toSiteMediaPath(importRow.photoRowDraft.thumbnail) : face.photo_url || ''),
      collection: String(
        getMetadataValue<string>(face, 'collection') || importRow?.collection || 'Uncategorized'
      ),
      storyLaneSuggestion:
        getMetadataValue<string>(face, 'storyLaneSuggestion') ||
        importRow?.storyLaneSuggestion ||
        null,
      duplicateGroupId:
        String(
          getMetadataValue<string>(face, 'duplicateGroupId') || importRow?.duplicateGroupId || ''
        ) || null,
      captureDate:
        String(
          getMetadataValue<string>(face, 'captureDate') || importRow?.photoRowDraft.date || ''
        ) || null,
      faces: [],
      pendingCount: 0,
      confirmedCount: 0,
...
async function readJsonArtifact<T>(
  ...
  const { data, error } = await import('@/lib/supabase').then(m =>
    m.downloadMediaReviewArtifact(batch.artifact_bucket, objectPath)
  )
  if (error || !data) {
    return null
  }
  ...
  devtools(
    (set, get) => ({
      ...
      getSelectedFace: () => {
        ...
        return (
          selectedPhoto?.faces.find(face => face.id === selectedFaceId) ||
          selectedPhoto?.faces[0] ||
          null
        )
      },
      ...
      loadBatches: async () => {
        ...
        set(() => ({
          batches: nextBatches,
          selectedBatchId:
            selectedBatchId && nextBatches.some(batch => batch.id === selectedBatchId)
              ? selectedBatchId
      ...
      loadBatchDetails: async (batch: MediaReviewBatch) => {
        ...
        set(state => ({
          ...
          faceDrafts: Object.fromEntries(
            nextFaces.map(face => [face.id, normalizeFaceDraft(face)])
          ),
      ...
      updateDraft: (faceId, patch) => {
        ...
        set(state => ({
          faceDrafts: {
            ...state.faceDrafts,
            [faceId]: {
              ...(state.faceDrafts[faceId] ||
                (face
                  ? normalizeFaceDraft(face)
                  : {
      ...
      stageGroupStatus: status => {
        ...
        get().updateFacesDrafts(selectedGroup.faceIds, { reviewStatus: status })
      },
      ...
      saveFaces: async faceIds => {
        ...
        try {
          ...
          for (const { face, draft } of changedFaces) {
            ...
            const personKey =
              draft.personKey.trim() || (confirmedName ? slugifyPerson(confirmedName) : '')
      ...
      handleBatchStatusChange: async (batch, status) => {
        ...
        set(state => ({
          batches: state.batches.map(item => (item.id === batch.id ? { ...item, status } : item)),
        }))
      ...
      loadCropPreviews: async () => {
        const { photoInspectorOpen } = get()
        const selectedBatch = get().getSelectedBatch()
        const selectedPhoto = get().getSelectedPhoto()
        const selectedGroup = get().getSelectedGroup()
