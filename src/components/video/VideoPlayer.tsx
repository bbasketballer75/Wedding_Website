...
import { 
  ...
  SkipBack, SkipForward, Cast, ChevronRight, X, RotateCw, Smartphone
} from 'lucide-react'
...
export function VideoPlayer({ 
  ...
  useEffect(() => {
    ...
    video.addEventListener('webkitcurrentplaybacktargetiswirelesschanged', syncCastState as EventListener)
    ...
    return () => {
      ...
      video.removeEventListener('webkitcurrentplaybacktargetiswirelesschanged', syncCastState as EventListener)
    }
  }, [])
          ...
          >
            <div className="max-w-sm rounded-[1.6rem] border border-gold-200/20 bg-[linear-gradient(145deg,rgba(38,28,22,0.94),rgba(54,39,31,0.95))] px-5 py-6 shadow-[0_24px_60px_-32px_rgba(21,20,19,0.72)] backdrop-blur-xl sm:px-6">
              ...
              <p className="mt-3 text-sm leading-6 text-cinematic-secondary">
                The full film uses a landscape-only player on phones so the controls and framing stay clean.
              </p>
            </div>
          </motion.div>
        )}
              ...
              <div className="max-w-full rounded-[1.2rem] border border-gold-200/12 bg-[linear-gradient(135deg,rgba(255,247,235,0.12),rgba(255,255,255,0.06))] px-3 py-3 backdrop-blur-md sm:max-w-[70%] sm:px-4">
                ...
                {resolvedChapters.length > 0 && (
                  ...
                  >
                    ...
                    <ChevronRight className={cn('mt-0.5 h-4 w-4 shrink-0 transition-transform', showChapterMenu && 'rotate-90')} />
                  </button>
                )}
              </div>
              ...
              <div className="flex items-center justify-end gap-2 sm:self-start">
                {canCast && (
                  <button 
                    ...
                    aria-label={isCasting ? 'Change casting or AirPlay target' : 'Cast or AirPlay this video'}
              ...
              {showChapterMenu && resolvedChapters.length > 0 && (
                ...
                >
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <h4 className="text-sm font-medium uppercase tracking-wider text-cinematic-primary">Chapters</h4>
                    <button
                      type="button"
