...
import { type MediaReviewBatch, type MediaReviewBatchStatus } from '@/lib/supabase'
...
export function BatchList({
  ...
  const changedGroupCount = (() => {
    ...
    return personGroups.filter(group => group.faceIds.some(faceId => changedFaceIdSet.has(faceId)))
      .length
  })()
      ...
      <section className='rounded-[1.4rem] border border-gold-100 bg-white p-4 shadow-sm'>
        <div className='flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between'>
          <div className='grid gap-4 lg:grid-cols-[18rem_minmax(0,1fr)] xl:min-w-0 xl:flex-1'>
            <div>
              <label
                htmlFor='review-batch-picker'
                className='mb-2 block text-[11px] uppercase tracking-[0.28em] text-charcoal-500'
              >
                Review batch
              </label>
            ...
            <div className='grid gap-3 rounded-[1rem] border border-gold-100 bg-cream-50/70 px-4 py-3 text-sm text-charcoal-600 sm:grid-cols-2 xl:grid-cols-4'>
              ...
              <div>
                <p className='text-[11px] uppercase tracking-[0.22em] text-charcoal-400'>
                  Pending faces
                </p>
              ...
              <div>
                ...
                <p className='mt-1 font-medium text-charcoal-900'>
                  {lastSavedSummary || 'No changes saved yet'}
                </p>
              </div>
            </div>
          </div>
        ...
        <details
          ...
          onToggle={event =>
            useMediaReviewStore.getState().setShowAdvancedTools(event.currentTarget.open)
          }
        >
          ...
          <div className='mt-4 flex flex-wrap gap-2'>
            <Button
              size='sm'
              variant='secondary'
              onClick={() => onBatchStatusChange(selectedBatch, 'pending')}
            >
