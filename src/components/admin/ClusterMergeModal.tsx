...
interface ClusterMergeModalProps {
  isOpen: boolean
  onClose: () => void
  selectedPhoto: { key: string; sourceRelativePath: string; photoUrl: string; faces: MediaReviewFace[] } | null
...
function getOverlayStyle(face: MediaReviewFace) {
  const dimensions = face.metadata && typeof face.metadata === 'object'
    ? (face.metadata as Record<string, unknown>)['detectionDimensions'] as Record<string, unknown> | undefined
    : undefined
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <h4 className="mt-2 text-lg font-medium text-charcoal-900">{getDraftFaceLabel(face, draft)}</h4>
