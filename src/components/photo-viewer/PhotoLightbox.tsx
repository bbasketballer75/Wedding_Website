            ...
            >
                    ...
                    <AnimatePresence mode='wait'>
                      <motion.img
                        ...
                        onClick={e => {
                          ...
                          if (isMouseEvent) {
                            doubleTapTimer.current = {
                              x: (touch as MouseEvent).clientX,
                              y: (touch as MouseEvent).clientY,
                              time: now,
                            }
                            setTimeout(() => {
                              if (
                                doubleTapTimer.current &&
                                Date.now() - doubleTapTimer.current.time >= 300
                              )
                                doubleTapTimer.current = null
                            }, 310)
                          }
                  ...
                  <div className='flex flex-wrap items-center justify-center gap-2 sm:gap-3'>
                    ...
                    >
                      ...
                      <span className='text-sm'>
                        {currentPhoto.commentCount ?? currentPhoto.comments?.length ?? 0}
                      </span>
                    </button>
                  </div>
                </div>
              ...
              <AnimatePresence>
                {showInfo && (
                  <motion.div
                    initial={{
                      x: typeof window !== 'undefined' && window.innerWidth < 640 ? 0 : 300,
                      y: typeof window !== 'undefined' && window.innerWidth < 640 ? 300 : 0,
                      opacity: 0,
                    }}
                    ...
                    <div className='flex-1 overflow-y-auto p-4 pb-8 sm:pb-4'>
                      {activeTab === 'info' ? (
                        <div className='space-y-4'>
                          {currentPhoto.caption && (
                            <div>
                              ...
                              <p className='text-sm leading-6 text-charcoal-400'>
                                Keep the focus on the image. Download, comment, or use the people
                                list only when you need the extra context.
                              </p>
                            </div>
                          )}
                          ...
                          {currentPhoto.faces && currentPhoto.faces.length > 0 && (
                            <div>
                              <h4 className='text-charcoal-400 text-sm mb-2'>
                                People in this photo
                              </h4>
                              <div className='flex flex-wrap gap-2'>
                                {currentPhoto.faces.map(face => {
                                  const isVerified = verifiedNames.includes(
                                    ...
                                    >
                                      ...
                                      <span
                                        className={cn(
                                          'text-sm font-sans',
                                          isVerified
                                            ? 'font-medium italic text-gold-500'
                                            : 'text-charcoal-300'
                                        )}
                                      >
                                        {face.name}
                          ...
                          {!currentPhoto.caption &&
                            !(currentPhoto.faces && currentPhoto.faces.length > 0) && (
                              <p className='text-sm leading-6 text-charcoal-400'>
                                Use the toolbar for download and comments. This panel stays
                                intentionally light unless the photo has people or a caption
                                attached.
                              </p>
                            )}
                        </div>
