...
import { X, Printer, Check, Copy, ExternalLink, Sparkles } from 'lucide-react'
...
export function PrintModal({ isOpen, onClose, photoUrl }: PrintModalProps) {
  ...
  const providers = [
    ...
    },
    {
      id: 'shutterfly',
      name: 'Shutterfly',
      description: 'Affordable classic prints, large canvas wraps, and custom personalized photo books.',
          ...
          >
            ...
            <div className="flex items-center justify-between pb-6 border-b border-white/5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 id="print-modal-title" className="font-serif text-2xl text-white">Order Prints</h3>
                  <p className="text-white/60 text-sm mt-0.5">Transform digital wedding memories into physical keepsakes</p>
                </div>
              </div>
              <button
                onClick={onClose}
                ...
                {copiedProvider ? (
                  ...
                  >
                    ...
                    <p className="text-white/70 text-xs mt-2 leading-relaxed max-w-md mx-auto">
                      Opening <strong>{copiedProvider}</strong> in a new tab. You can paste this URL (using <kbd className="bg-white/10 px-1 py-0.5 rounded text-[10px]">Ctrl+V</kbd> or <kbd className="bg-white/10 px-1 py-0.5 rounded text-[10px]">⌘+V</kbd>) directly into their photo uploader!
                    </p>
                  </motion.div>
                ) : (
                  <div className="rounded-xl border border-white/5 bg-white/5 p-4 flex gap-3 text-white/50 text-xs leading-relaxed">
                    ...
                    <p>
                      External print sites cannot fetch private photos directly. When you select a provider, we copy this photo's direct high-resolution URL to your clipboard. Once their website opens, simply paste the clipboard link into their image uploader!
                    </p>
                  </div>
                )}
              </AnimatePresence>
