  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-charcoal-900/60 backdrop-blur-md">
        ...
        <div className="absolute inset-0" onClick={closeWizard} />
          ...
          <div className="p-6 overflow-y-auto flex-1 font-sans">
            ...
            {step === 1 && (
              <form onSubmit={handleSendOtp} className="space-y-4">
                <div className="text-sm text-charcoal-600 leading-relaxed mb-4">
                  {targetFaceId ? (
                    <span>
                      Claiming face cluster for <strong>"{targetFaceName}"</strong>. Please confirm your guest name and email address to receive a verification code.
                    </span>
                  ) : (
                    <span>
                      Claim photos you previously uploaded to the wedding archive. Enter your name and the email address used during upload to authenticate.
                    </span>
                  )}
                </div>
            ...
            {step === 2 && (
              <form onSubmit={handleOtpVerify} className="space-y-4">
                <div className="text-sm text-charcoal-600 leading-relaxed mb-4">
                  We've sent a 6-digit verification code to <strong>{email}</strong>. Please enter the code below to verify your identity.
                </div>
              ...
              <div className="space-y-4">
                <div className="text-sm text-charcoal-600 leading-relaxed mb-2">
                  {photosToClaim.length > 0 ? (
                    <span>
                      We found <strong>{photosToClaim.length}</strong> photo(s) matching your uploads. Select the ones you want to claim under your verified identity.
                    </span>
                  ) : (
                    <span>
                      Confirm your photo claim request. Click below to submit your verified identity claim to Austin & Jordyn's moderation team.
                    </span>
                  )}
                </div>
                          ...
                          >
                            ...
                            <div
                              className={`absolute top-1.5 right-1.5 w-5 h-5 rounded-full flex items-center justify-center transition-all ${
                                isSelected ? 'bg-gold-500 text-cream-50 scale-100' : 'bg-charcoal-900/40 text-transparent scale-0'
                              }`}
                            >
                ...
                {targetFaceId && (
                  <div className="p-3 bg-gold-500/5 border border-gold-500/20 rounded-lg flex items-start gap-2.5 text-xs text-gold-800">
                    ...
                    <div>
                      ...
                      This will automatically link face clusters tagged as <strong>"{targetFaceName}"</strong> directly to your verified guest profile.
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-3 mt-6">
                  ...
                  <button
                    ...
                    disabled={isLoading || (photosToClaim.length > 0 && selectedPhotoIds.length === 0)}
                    className="flex-[2] py-2.5 bg-gold-500 hover:bg-gold-600 disabled:bg-gold-300 text-cream-50 font-semibold rounded-lg shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-1.5 cursor-pointer text-sm"
                  >
                    {isLoading ? (
                      <Loader2 className="w-4 h-4 animate-spin" />
              ...
              <div className="text-center py-6 space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-charcoal-600 leading-relaxed max-w-md mx-auto">
                    Thank you, <strong>{displayName}</strong>! Your claim has been sent to Austin & Jordyn's admin moderation queue.
                  </p>
                </div>

                <div className="p-4 bg-cream-100/50 rounded-xl max-w-sm mx-auto text-xs text-charcoal-500 border border-gold-500/5">
                  Once the admin approves, your name will be rendered in a gorgeous gold italic font inside the Photo Lightbox info details panel.
                </div>
