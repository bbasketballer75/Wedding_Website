...
import { MAP_CONFIG, VIDEO_CONFIG } from '@/config/constants'
...
const RecenterMap: React.FC<{ pins: GuestPin[] }> = ({ pins }) => {
  const map = useMap()
  ...
  useEffect(() => {
    ...
    if (validPins.length > 0) {
      ...
      map.fitBounds(bounds, { padding: MAP_CONFIG.padding as [number, number] })
    }
  }, [pins, map])
...
const GuestMap: React.FC = () => {
  ...
  const [myPinId, setMyPinId] = useState<string | null>(storage.getItem(VIDEO_CONFIG.guestPinStorageKey))
