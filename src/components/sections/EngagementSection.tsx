...
const EngagementSection = () => {
  const [selectedPhoto, setSelectedPhoto] = useState<EngagementPhoto | null>(null)
  const sectionRef = useRef<HTMLElement>(null)
  const isInView = useInView(sectionRef, { amount: 0, margin: '0px 0px -20% 0px' })

  const mainPhoto = photos.find(p => p.type === 'main') as EngagementPhoto | undefined
  const secondaryPhoto = photos.find(p => p.type === 'filler') as EngagementPhoto | undefined
  ...
  return (
    ...
    >
      ...
      {/* Spooky Decorations - Only render when in view */}
      {isInView && (
        <>
            ...
            <div className='grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center relative z-10'>
              ...
              >
                <div className='relative z-10'>
                  ...
                  {storyContent.text.map((paragraph: string, index: number) => (
                    <p
                      key={index}
