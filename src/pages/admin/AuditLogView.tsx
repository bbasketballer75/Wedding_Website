...
export function AuditLogView() {
  ...
  const [entityFilter, setEntityFilter] = useState<'all' | 'guest_upload' | 'guestbook_message'>(
    'all'
  )
  ...
  const actorOptions = useMemo(() => {
    return Array.from(
      new Set(
        entries.map(entry => entry.actor_email).filter((value): value is string => Boolean(value))
      )
    )
  }, [entries])
  ...
  const filteredEntries = useMemo(() => {
    ...
    return entries.filter(entry => {
      ...
      const haystack =
        `${entry.summary} ${entry.actor_name || ''} ${entry.actor_email || ''} ${metadataText}`.toLowerCase()
    ...
    <div className='space-y-6'>
      <div className='space-y-2'>
        ...
        <p className='max-w-3xl text-sm leading-6 text-charcoal-500'>
          Every recorded moderation action lives here. Use it to confirm who approved, rejected,
          published, or removed content and when those decisions happened.
        </p>
      </div>
