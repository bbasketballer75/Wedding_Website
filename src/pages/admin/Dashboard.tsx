...
import { supabase, fetchMediaReviewBatches, fetchGuestFaceTaggingBatches } from '@/lib/supabase'
...
function StatCard({
  ...
  return (
    <div
      className={`bg-white rounded-[1.2rem] p-4 shadow-sm border ${alert ? 'border-amber-400' : 'border-gold-100'}`}
    >
...
export function Dashboard() {
  ...
  useEffect(() => {
    async function fetchStats() {
      ...
      const peopleBatchesInFlight = (mediaReviewBatches || []).filter(
        batch => batch.status === 'pending' || batch.status === 'in_review'
      ).length
          ...
          <div className='flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between'>
            <div className='max-w-3xl'>
              ...
              <p className='mt-2 text-sm leading-6 text-charcoal-600'>
                Use this like a briefing, not a landing page: check the current state, then jump
                straight into the next workflow.
              </p>
            </div>
          ...
          <div className='rounded-[1.5rem] border border-gold-100 bg-white p-5 shadow-sm'>
            <div className='flex items-center justify-between gap-4'>
              <div>
                ...
                <h3 className='mt-1 text-xl font-display text-charcoal-900'>
            ...
            <div className='mt-4 grid gap-3 lg:grid-cols-3'>
              <WorkflowStep step='1' title='Photos' description='Clear /admin/photos first.' />
              <WorkflowStep step='2' title='People' description='Tighten names in /admin/review.' />
              <WorkflowStep
                step='3'
                title='Guestbook'
