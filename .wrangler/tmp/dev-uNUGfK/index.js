...
var media_rewrite_default = {
  async fetch(request, env) {
    ...
    rewrittenPath = rewrittenPath.replace(
      /^_thumbs\/Professional\/Wedding ?Day\/Photos\//,
      "professional/photos/wedding-photos/"
    );
    rewrittenPath = rewrittenPath.replace(
      /^_thumbs\/Professional\/Bach[ +]ette\/Photos\//,
      "professional/photos/bachelor-ette/"
    );
    rewrittenPath = rewrittenPath.replace(
      /^_thumbs\/Bach[ +]ette\/Photos\//,
      "professional/photos/bachelor-ette/"
    );
    rewrittenPath = rewrittenPath.replace(
      /^_thumbs\/Engagement\/Photos\//,
      "professional/photos/proposal/"
    );
    rewrittenPath = rewrittenPath.replace(
      /^_thumbs\/Professional\/([^/]+)\/Photos\//,
      "professional/photos/$1/"
    );
    rewrittenPath = rewrittenPath.replace(
      /^_thumbs\/Guest ?Uploads\/Wedding ?Day\/Live ?Photos\/Stills\//,
      "guest-uploads/wedding-day/live-photos/stills/"
    );
    if (rewrittenPath.startsWith("/_thumbs/")) {
      rewrittenPath = rewrittenPath.replace("/_thumbs/", "/");
    }
    try {
